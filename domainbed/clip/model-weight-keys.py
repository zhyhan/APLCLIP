...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, key_padding_mask: torch.Tensor = None):
        return self.resblocks([x, key_padding_mask])
...
class VisionTransformer(nn.Module):
    ...
    def random_remove_patches(self, input_tensor, mask_ratio=0.5):
        ...
        # 生成随机索引（除了第一个patch）可以将学到的indices都加上1，避免删除class token
        indices = torch.randperm(patch_size - 1)[:remaining_patches - 1] + 1

        # 保留第一个patch
        first_patch = input_tensor[:, 0:1, :]

        # 选择需要保留的patch
        remaining_patches = input_tensor[:, indices, :]

        # 合并第一个patch和剩余的patches
        output_tensor = torch.cat((first_patch, remaining_patches), dim=1)
    ...
    def get_mask(self, indices):
        #indices = indices[:, 0]=True#let the weight of class token large also. 
        attention_mask = torch.zeros_like(indices).to(torch.float)
        attention_mask[indices]=1.

        return attention_mask


    def forward(self, x: torch.Tensor, z:torch.Tensor, train=False):
        ...
        if train:
            attention_mask = self.get_mask(z)
            x = self.ln_pre(x)
            x = x.permute(1, 0, 2)  # NLD -> LND
            x = self.transformer(x, attention_mask)#mask is a N, L
            x = x[0]
