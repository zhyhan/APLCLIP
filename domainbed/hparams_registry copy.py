...
def _hparams(algorithm, dataset, random_seed):
    ...
    _hparam('backbone', 'clip', lambda r: 'resnet50')
    ...
    if algorithm in ['DANN', 'CDANN', 'APLCLIP']:
    ...
    if dataset in SMALL_IMAGES or algorithm in ["DPLCLIP"]:  # DPLCLIP using SGD follower prior work.
        ...
        _hparam('lr', 1e-3, lambda r: 10**r.uniform(-4.5, -2.5))#TODO: adjust learning rate
    ...
    if algorithm in ['DANN', 'CDANN', 'APLCLIP'] and dataset in SMALL_IMAGES:
        _hparam('lr_g', 1e-5, lambda r: 10**r.uniform(-4.5, -2.5) )
    elif algorithm in ['DANN', 'CDANN', 'APLCLIP']:
