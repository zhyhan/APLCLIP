...
from domainbed import utils as ut
...
class DPLCLIP(CLIP):
    def __init__(self, input_shape, num_classes, num_domains, hparams, sentence_prompt=False):
        ...
        if sentence_prompt:
            ...
            #classnames = [f"a {name.replace('_', ' ')} cause" for name in hparams['class_names']]
            classnames = [f"a photo of a {name.replace('_', ' ')}" for name in hparams['class_names']]
        else:
            classnames = [name.replace('_', ' ') for name in hparams['class_names']]
        #prompts = [name + ' ' + 'cause' + ' ' + prompt_prefix + '.' for name in classnames]
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        image_features = torch.cat(image_features)#将多个domain的feature变成一个长feature
        #  reshape [self.batch_size, self.EMBEDDING_DIM.]:  -> [1, self.EMBEDDING_DIM.]
        mean_domain_features = [feature.mean(dim=0, keepdim=True) for feature in domain_features]#将每一个domain的feature按照batch进行平均化.维度为[1, 8192]. 其中8192=16*512.
        #16个token，每个token的维度为512

        #  reshape [1, self.EMBEDDING_DIM.]:  -> [7, self.EMBEDDING_DIM.]扩充类别数倍.
...
# class CausalVAE(Algorithm):
#     """reimplement CausalVAE"""

#     def __init__(self, input_shape, num_classes, num_domains, hparams):
#         super(CausalVAE, self).__init__(input_shape, num_classes, num_domains,
#                                   hparams)
#         #Form CLIP obtain image features
#         self.featurizer = networks.Featurizer(input_shape, self.hparams)


class CausalVAE(torch.nn.Module):
    def __init__(self, input_shape, num_classes, num_domains, hparams, name='vae', z_dim=512, z1_dim=1, z2_dim=512, inference = False):
        super().__init__()
        self.name = name
        self.z_dim = z_dim
        self.z1_dim = z1_dim
        ...
        self.enc = networks.Featurizer(input_shape, self.hparams)
        self.dec = networks.Decoder_DAG(self.z_dim, self.z1_dim, self.z2_dim)
        self.dag = networks.DagLayer(self.z1_dim, self.z1_dim, i = inference)
        self.attn = networks.Attention(self.z2_dim)
        self.mask_z = networks.MaskLayer(self.z_dim,concept=self.z1_dim)
        self.mask_u = networks.MaskLayer(self.z1_dim,concept=self.z1_dim,z1_dim=1)
    ...
    def negative_elbo_bound(self, x, label, mask = None, sample = False, adj = None, lambdav=0.001):
        ...
        """
        assert label.size()[1] == self.z1_dim #z1_dim表示要学习的causal factors的维度

        q_m, q_v = self.enc.encode(x)#使用CLIP image encoder.
        q_m, q_v = q_m.reshape([q_m.size()[0], self.z1_dim,self.z2_dim]),torch.ones(q_m.size()[0], self.z1_dim,self.z2_dim)

        decode_m, decode_v = self.dag.calculate_dag(q_m, torch.ones(q_m.size()[0], self.z1_dim,self.z2_dim))
        decode_m, decode_v = decode_m.reshape([q_m.size()[0], self.z1_dim,self.z2_dim]),decode_v
        if sample == False:
          if mask != None  and mask in [0,1, 3]:
              z_mask = torch.ones(q_m.size()[0], self.z1_dim,self.z2_dim)*adj
              decode_m[:, mask, :] = z_mask[:, mask, :]
              decode_v[:, mask, :] = z_mask[:, mask, :]
          m_zm, m_zv = self.dag.mask_z(decode_m).reshape([q_m.size()[0], self.z1_dim,self.z2_dim]),decode_v.reshape([q_m.size()[0], self.z1_dim,self.z2_dim])
          m_u = self.dag.mask_u(label)
          #mask
          
          f_z = self.mask_z.mix(m_zm).reshape([q_m.size()[0], self.z1_dim,self.z2_dim])
          
          e_tilde = self.attn.attention(decode_m.reshape([q_m.size()[0], self.z1_dim,self.z2_dim]),q_m.reshape([q_m.size()[0], self.z1_dim,self.z2_dim]))[0]
          ...
          m_zv = torch.ones([q_m.size()[0],self.z1_dim,self.z2_dim])
          z_given_dag = ut.conditional_sample_gaussian(f_z1, q_v*lambdav)
          
        decoded_bernoulli_logits,x1,x2,x3,x4 = self.dec.decode_sep(z_given_dag.reshape([z_given_dag.size()[0], self.z_dim]), label)
        ...
        cp_v = torch.ones([q_m.size()[0],self.z1_dim,self.z2_dim])
        cp_z = ut.conditional_sample_gaussian(cp_m, cp_v)
        kl = torch.zeros(1)
        kl = 0.3*ut.kl_normal(q_m.view(-1,self.z_dim), q_v.view(-1,self.z_dim), p_m.view(-1,self.z_dim), p_v.view(-1,self.z_dim))
        
        for i in range(self.z1_dim):
            kl = kl + 1*ut.kl_normal(decode_m[:,i,:], cp_v[:,i,:],cp_m[:,i,:], cp_v[:,i,:])
        kl = torch.mean(kl)
        mask_kl = torch.zeros(1)
        mask_kl2 = torch.zeros(1)
        for i in range(self.z1_dim):
            mask_kl = mask_kl + 1*ut.kl_normal(f_z1[:,i,:], cp_v[:,i,:],cp_m[:,i,:], cp_v[:,i,:])
        u_loss = torch.nn.MSELoss()
        mask_l = torch.mean(mask_kl) + u_loss(g_u, label.float())
        nelbo = rec + kl + mask_l
...
class AbstractDANN(Algorithm):
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        if self.conditional:
            disc_input = all_z + self.class_embeddings(all_y)#TODO: use clip的text encoder.
        else:
            disc_input = all_z
...
class GroupDRO(ERM):
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        if not len(self.q):
            self.q = torch.ones(len(minibatches))
