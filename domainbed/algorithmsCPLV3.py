...
class APLCLIP(CLIP):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        self.prompt_opt = torch.optim.AdamW(#only update little parameters, knowledge vit structures
        ...
        self.gen_scheduler = CosineAnnealingLR(self.gen_opt, T_max=5000, eta_min=1e-7, last_epoch=-1, verbose=False)
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        #remove zero tensores
        #unsqueeze_image_features = image_features.unsqueeze(dim=1)
        # unsqueeze_image_features_expand = unsqueeze_image_features.expand(-1, 3, -1)
        # unsqueeze_image_features_expand = unsqueeze_image_features_expand.reshape(unsqueeze_image_features.size(0), -1)
        # all_acl_view = all_acl.reshape(all_acl.size(0), -1)
        # zero_indices = torch.all(all_acl_view == 0, axis=1)
        # all_acl_view[zero_indices] = unsqueeze_image_features_expand[zero_indices]
        acl_features = torch.cat([all_acl, 0.1*image_features.unsqueeze(dim=1)], dim = 1)

        #acl_features = acl_features.mean(dim=0, keepdim=True) #consider the computation speed
        ctx = self.prompt_generator(ctx_only=True)

        #N = image_features.size(0) #每个样本都要算一个独立的text encoder, 虽然这样会减慢速度。
        N = 1
        ...
        disc_input = acl_features.mean(dim=1, keepdim=False) #consider the computation speed

        #disc_input = acl_features
        ...
        if (self.update_count.item() % (1+d_steps_per_g) < d_steps_per_g):#偶数
            ...
            # margin = 0.2
            # dist_loss = distences_per_image.mean()-margin
            #dist_loss = torch.max(distences_per_image.mean()-margin, 0)
    ...
    def predict(self, x, z):
        ...
        #remove zero tensores
        #unsqueeze_image_features = image_feature.unsqueeze(dim=1)
        # unsqueeze_image_features_expand = unsqueeze_image_features.expand(-1, 3, -1)
        # unsqueeze_image_features_expand = unsqueeze_image_features_expand.reshape(unsqueeze_image_features.size(0), -1)
        # all_acl_view = all_acl.reshape(all_acl.size(0), -1)
        # zero_indices = torch.all(all_acl_view == 0, axis=1)
        # all_acl_view[zero_indices] = unsqueeze_image_features_expand[zero_indices]
        #acl_features = torch.cat([all_acl_view, image_feature], dim = 1)
        #acl_features = torch.cat([all_acl.view_as(all_acl), image_feature.unsqueeze(dim=1)], dim = 1)

        prompts = self.prompt_generator()
        prompts = prompts.expand(image_feature.size(0), -1, -1, -1)
...
class CoCoOpPromptLearner(CLIP):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        n_ctx = 4
        ctx_init = "some_useful_intristic_attributes"
    ...
    def forward(self, ctx_only=False):
        ...
        ctx_i = ctx_shifted.expand(self.n_cls, -1, -1)
