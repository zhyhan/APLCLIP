...
class APLCLIP(CLIP):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        # )
        #增加learning rate decay
        self.gen_opt = torch.optim.Adam(
            (list(self.prompt_generator.meta_net.parameters()) + list(self.bottle_neck.parameters()) + [self.prompt_generator.ctx]),
        ...
        #self.scheduler = StepLR(self.gen_opt, step_size=300, gamma=0.05)

    def adjust_learning_rate_poly(self, optimizer, initial_lr, iteration, max_iter):
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        image_features = self.bottle_neck(image_features)
        ...
        if (self.update_count.item() % (1+d_steps_per_g) < d_steps_per_g):#偶数
            ...
            gen_loss.backward()
            self.gen_opt.step()
            lr = self.adjust_learning_rate_poly(self.gen_opt, self.hparams["lr_g"], self.update_count.item(), 5000)
            #self.scheduler.step()
    ...
    def predict(self, x, z):
        ...
        image_feature = self.bottle_neck(image_feature)
        prompts, _ = self.prompt_generator(acl_features)
